...
const AdminJobList = () => {
  ...
  useEffect(() => {
    fetchAdminJobs(); // Fetch all jobs as admin
  }, []);
  ...
  useEffect(() => {
    setPageIndex(0); // Reset to first page on filter/search
  }, [search, statusFilter]);
  ...
  const columns = useMemo(
    () => [
      ...
      // NOTE: The Actions column (Edit/Delete) is currently Show  intentionally.
    ...
    [openModal, deleteJob, setSelectedJob]
  );
    ...
    <>
      <div className='container mt-4'>
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h3>All Jobs <small className="text-muted">(Admin View)</small></h3>
          {/* No Create Button for admin */}
        </div>

        {user?.role === 'admin' && (
          <div className="alert alert-info" role="alert">
            <strong>Note:</strong> The "Actions" column (Edit/Delete) is currently Show.
            Admin functionality is integrated. Comment code in the table file if not needed.
          </div>
        )}
